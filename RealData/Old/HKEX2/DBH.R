...
# calculate the value of QL values given the pre-specific breakpoints
# INPUT:
#  @Y: the observation matrix (T-by-p)
#  @r: the number of factors (scalar)
#  @plfG: estimated factor matrix (T-by-r)
#  @Ks: the given break date (L-dim vector)
# OUTPUT:
#  @value: the value of QL function
DBHQML <- function(Y,r,plfG,Ks){
  N <- nrow(Y); p <- ncol(Y)
  ...
  for (ll in 1:(L+1)) {
    ...
    value <- value-Delta*log(det(as.matrix(SigmaG[,,ll])))
  }
  return(value)
}

# estimation for change points
# INPUT:
#  @Y: the observation matrix (T-by-p)
#  @L: the number of break dates (1 or 2)
# OUTPUT:
#  @Khat: the estimated CP
DBHQML.CP <- function(Y,L){
  ...
  if(L==1){
    ...
    for (tt in (N*0.25):(N*0.75)) {
      Ks <- c(tt)
      li.value[tt] <- DBHQML(Y,r,Ghat,Ks)
    }
  }
  ...
  if(L==2){
    ...
    for (tt in (N*0.25):(N*0.75-(0.15*N))) {
      for (pp in (tt+(0.15*N)):(N*0.75)) {
        Ks <- c(tt,pp)
        li.value[tt,pp] <- DBHQML(Y,r,Ghat,Ks)
      }
    }
  }
  
  li.value[is.na(li.value)] <- -Inf
  #li.value[li.value==Inf] <- -Inf
  Khat <- which(li.value==max(li.value), arr.ind = TRUE)
